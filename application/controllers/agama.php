...
class Agama extends CI_Controller {
	...
	public function add($id = "")
	{
		...
		$form_manage = $this->ktp_model->display_form_agama($id);
	...
	public function edit($id)
	{
		...
		if(isset($ss['ista_adminid'])):
			...
			$datax = array('url' => base_url(),
						   'title' => 'Ubah Profil',
						   '_templateView_' => $this->ktp_model->display_form_agama($id),
						   '_sidebarMenu_' => $this->general_model->sidebarMenu(),
						   '_topNavigation_' => $this->general_model->topNavigation(),
						   '_menuFooter_' => $this->general_model->menuFooter(),
	...
	{	
		$rs=$this->ktp_model->get_agama($id);

		$this->db->delete('agama', array('id_agama' => $id));		
	...
	public function remove()
	{
		...
		if(!empty($chk)):
			...
			{
				$this->db->delete('agama', array('id_agama' => $chk[$x]));
			}
	...
	public function save()
	{
		...
		$data['nama_agama'] = $post['nama'];

		 $id=$post['id'];
		 if(empty($id)){
			$this->db->insert('agama', $data);
		 } else {
		 	$this->db->update('agama', $data, array('id_agama' => $post['id']));
		 }
	...
	$ss = $this->session->userdata('ista_logged');
		...
		if(isset($ss['ista_adminid'])):
			...
			$datax = array('url' => base_url(),
						   'title' => 'DATA AGAMA',
						   '_templateView_' => $this->ktp_model->display_master_agama(),
						   '_sidebarMenu_' => $this->general_model->sidebarMenu(),
						   '_topNavigation_' => $this->general_model->topNavigation(),
						   '_menuFooter_' => $this->general_model->menuFooter(),
						   'judul' => 'DATA AGAMA',

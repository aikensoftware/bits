...
class Pendidikan extends CI_Controller {
	...
	public function add($id = "")
	{
		...
		$form_manage = $this->ktp_model->display_form_pendidikan($id);
	...
	public function edit($id)
	{
		...
		if(isset($ss['ista_adminid'])):
			...
			$datax = array('url' => base_url(),
						   'title' => 'Ubah Profil',
						   '_templateView_' => $this->ktp_model->display_form_pendidikan($id),
						   '_sidebarMenu_' => $this->general_model->sidebarMenu(),
						   '_topNavigation_' => $this->general_model->topNavigation(),
						   '_menuFooter_' => $this->general_model->menuFooter(),
	...
	{	
		$rs=$this->ktp_model->get_pendidikan($id);
	...
	public function remove()
	{
		...
		if(!empty($chk)):
			...
			{
				$this->db->delete('pendidikan', array('id_pendidikan' => $chk[$x]));
			}
	...
	public function save()
	{
		...
		$data['nama_pendidikan'] = $post['nama'];

		 $id=$post['id'];
		 if(empty($id)){
			$this->db->insert('pendidikan', $data);
		 } else {
		 	$this->db->update('pendidikan', $data, array('id_pendidikan' => $post['id']));
		 }
	...
	$ss = $this->session->userdata('ista_logged');
		...
		if(isset($ss['ista_adminid'])):
			...
			$datax = array('url' => base_url(),
						   ...
						   '_templateView_' => $this->ktp_model->display_master_pendidikan(),
						   '_sidebarMenu_' => $this->general_model->sidebarMenu(),
						   '_topNavigation_' => $this->general_model->topNavigation(),
						   '_menuFooter_' => $this->general_model->menuFooter(),
